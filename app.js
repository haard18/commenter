...
app.get("/auth/linkedin", (req, res) => {
  ...
  if (!LINKEDIN_CONFIG.clientId || !LINKEDIN_CONFIG.clientSecret) {
    return res.status(500).json({
      error:
        "LinkedIn OAuth not configured. Please set LINKEDIN_CLIENT_ID and LINKEDIN_CLIENT_SECRET in .env",
    });
  }

  const state = Math.random().toString(36).substring(7); // Simple state parameter
  const authUrl = `${LINKEDIN_CONFIG.authUrl}?response_type=code&client_id=${
    LINKEDIN_CONFIG.clientId
  }&redirect_uri=${encodeURIComponent(
    LINKEDIN_CONFIG.redirectUri
  )}&state=${state}&scope=${encodeURIComponent(LINKEDIN_CONFIG.scope)}`;
  ...
  try {
    ...
    try {
      const profileResponse = await axios.get(
        `${LINKEDIN_CONFIG.apiUrl}/userinfo`,
        {
          headers: {
...
app.get("/me", async (req, res) => {
  ...
  try {
    ...
    try {
      ...
      console.log(
        "ℹ️  Userinfo endpoint not accessible, trying alternative..."
      );

      // Fallback: try to get profile from people endpoint
      const peopleResponse = await axios.get(
        `${LINKEDIN_CONFIG.apiUrl}/people/~`,
        {
          headers: {
            Authorization: `Bearer ${accessToken}`,
    ...
    res.json({
      ...
      message:
        "Profile retrieved. If URN is not available, you can manually set LINKEDIN_PROFILE_URN in .env",
    ...
    if (error.response?.status === 401) {
      ...
      return res.status(401).json({
        error:
          "Access token expired or invalid. Please re-authenticate at /auth/linkedin",
      });
    }
...
async function resolveUgcPostUrn(urn) {
  if (urn.startsWith("urn:li:ugcPost:") || urn.startsWith("urn:li:share:")) {
    // Already a post URN, return as-is
    return urn.replace("share", "ugcPost"); // in case it's a share URN
  }

  if (urn.startsWith("urn:li:activity:")) {
    // Fetch the activity to get the share/ugcPost URN
    const response = await axios.get(
      `${LINKEDIN_CONFIG.apiUrl}/activities/${encodeURIComponent(urn)}`,
      {
        headers: {
          Authorization: `Bearer ${accessToken}`,
          "X-Restli-Protocol-Version": "2.0.0",
    ...
    // Most activity objects include a shareUrn
    const shareUrn = response.data.shareUrn;
    if (!shareUrn)
      throw new Error("No share/ugcPost URN found in activity object");

    // Convert share URN to ugcPost URN
    return shareUrn.replace("share", "ugcPost");
  }

  throw new Error("Unsupported URN type. Must be ugcPost, share, or activity");
}
...
app.post("/comment", async (req, res) => {
  ...
  if (!urn || !comment) {
    return res.status(400).json({
      error:
        'Missing required fields. Please provide both "urn" and "comment" in the request body.',
    });
  }
  ...
  if (!actor) {
    return res.status(400).json({
      error:
        'No actor URN available. Please provide "actorUrn" in request body or visit /me to set profile URN.',
  ...
  try {
    ...
    const targetUrn = await resolveUgcPostUrn(urn);
    ...
    const response = await axios.post(
      `${LINKEDIN_CONFIG.apiUrl}/socialActions/${encodeURIComponent(
        targetUrn
      )}/comments`,
      commentData,
      {
    ...
    if (error.response?.status === 403) {
      return res.status(403).json({
        error:
          "Permission denied. Make sure you have permission to comment on this post and your LinkedIn app has the correct permissions.",
      });
    }
...
app.listen(3001, () => {
  ...
  console.log(
    "4. Use POST http://localhost:3000/comment with JSON: {urn, comment}"
  );
  console.log("");
